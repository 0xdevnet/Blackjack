...
//import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/tree/v3.3.0/contracts/proxy/Initializable.sol";
//import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/tree/v3.3.0/contracts/access/OwnableUpgradeable.sol";
//import "https://github.com/OpenZeppelin/openzeppelin-contracts-upgradeable/tree/v3.3.0/contracts/math/SafeMathUpgradeable.sol";


/// @title A blackjack game
/// @author Clark Henry
/// @notice This contract has known security risks and should not be deployed in production
contract Blackjack is OwnableUpgradeable, usingProvable {
    ...
    receive() external payable {
        ...
        maxBet = SafeMathUpgradeable.div(address(this).balance, 200);
    }
    ...
    function doubleDown() public payable eitherStage(Stage.PlayHand, Stage.PlaySplitHand) {
        ...
        if (game.stage == Stage.PlayHand) {
            drawCard(game, game.player);
            game.player.doubleDownBet = SafeMathUpgradeable.add(game.player.doubleDownBet, msg.value);
            game.player.score = recalculate(game.player);


        } else if (game.stage == Stage.PlaySplitHand) {
            drawCard(game, game.splitPlayer);
            game.splitPlayer.doubleDownBet = SafeMathUpgradeable.add(game.splitPlayer.doubleDownBet, msg.value);
            game.splitPlayer.score = recalculate(game.splitPlayer);
        }
    ...
    function concludeGame(Game storage game) private stopInEmergency {
        ...
        if (game.player.score <= 21) {
            payout = SafeMathUpgradeable.add(payout, calculatePayout(game, game.player, dealerHasBJ) );
        }

        if (game.splitPlayer.score <= 21) {
            payout = SafeMathUpgradeable.add(payout, calculatePayout(game, game.splitPlayer, dealerHasBJ) );
        }

        require(payout <= SafeMathUpgradeable.mul(game.player.bet, 8), "Dealer error - payout is too high."); // 2 hands * double down = 4 bets max
    ...
    function calculatePayout(Game storage game, Player storage player, bool dealerHasBJ) private view returns (uint256 payout) {
        ...
        if (player.score == 21 && player.hand.length == 2 && !dealerHasBJ) {
            payout = SafeMathUpgradeable.div(SafeMathUpgradeable.mul(player.bet, 5), 2); // Blackjack pays 3:2
        } else if (player.score > dealer.score || dealer.score >= 21) {
            payout = SafeMathUpgradeable.mul(SafeMathUpgradeable.add(player.bet, player.doubleDownBet), 2);
        } else if (player.score == dealer.score) {
            payout = SafeMathUpgradeable.add(player.bet, player.doubleDownBet);
        } else {
            payout = 0;
        }
    }
